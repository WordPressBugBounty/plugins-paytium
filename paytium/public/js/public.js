...
(function ($) {
    ...
    function initializePaytiumForm(ptForm) {
        // Check if already initialized using data (which is not copied on simple clone)
        // We do not rely on the class 'pt-form-initialized' because Elementor might clone the HTML including the class,
        // but without the events/data.
        if (ptForm.data('pt-initialized')) {
            return;
        }
        ptForm.data('pt-initialized', true);
        ptForm.addClass('pt-form-initialized');

        var ptFormCurrencySymbol = ptForm.data('currency') && getCurrencySymbol(ptForm.data('currency')) !== undefined ?
            ...
            ptFormAmounts = [],
            ptFormFb = ptForm.hasClass('pt-fb-form');
        ...
        if (ptForm.find('input[name="pt-paytium-js-enabled"]').length === 0) {
        ...
        if (ptForm.find("[id^=pt-subscription-first-payment]").length > 0) {
            ...
            if (ptForm.find('.pt-first-payment-amount').length === 0) {
        ...
        // Make sure Yes is checked for subscription optional on page load, especially for back button
        ...
        ptForm.find( '.pt-quantity-input' ).on( 'keyup mouseup change', function (e) {
        ...
        function quantity_input_validation($quantity_input) {
            ...
            update_checkbox_field($quantity_input)
        ...
        function update_checkbox_field($quantity_input) {
            ...
            var $checkbox = $quantity_input.length ? (ptFormFb ? $quantity_input.parent().siblings(':checkbox:checked') : $quantity_input.siblings(':checkbox')) : $(this);
            ...
            $checkbox.parents('.pt-form-group-checkbox-new').find(':checked').each(function (index, element) {
                let cbQty = $quantity_input.length && $(element).siblings('paytium-quantity-input') ? $(element).siblings('.paytium-quantity-input').children('.pt-quantity-input') : false;
                total += cbQty ? parseFloat($(element).data('pt-price')) * parseFloat(cbQty.val()) : parseFloat($(element).val());
            });
        ...
        $(ptForm).off('submit').on('submit', function(e) {
        ...
        ptForm.find('.pt-payment-btn').off('click.ptPaymentBtn').on('click.ptPaymentBtn', check_item_limits);
        ...
        var ptPaytiumDate = ptForm.find('.pt-paytium-date');
        if (ptPaytiumDate.length > 0) {
            ptPaytiumDate.each(function() {
        ...
        var ptPaytiumBirthday = ptForm.find('.pt-paytium-birthday');
        if (ptPaytiumBirthday.length > 0) {
            ptPaytiumBirthday.each(function() {
                if (!$(this).hasClass('hasDatepicker')) {
                    $(this).datepicker({
                        dateFormat: 'dd-mm-yy',
                        changeMonth: true,
    ...
    $(function () {
        ...
        $body.on('change', '.pt-checkout-form .pt-field-checkbox', function () {
            var checkbox = $(this);
        ...
        // Function to initialize any uninitialized forms
        function initializeUninitializedForms() {
            $('.pt-checkout-form').each(function () {
                initializePaytiumForm($(this));
            });
        }

        // Initialize forms on initial page load
        initializeUninitializedForms();

        // Periodically check for and initialize new forms (e.g., in Elementor popups)
        setInterval(initializeUninitializedForms, 500); // Check every 500 milliseconds
    });
    ...
    $(document).ready(function () {
        ...
        $('body').on('click', '.paytium-spinner.increment', function () {
        ...
        $('body').on('click', '.paytium-spinner.decrement', function () {
            let input = $(this).siblings('.pt-quantity-input[type=number]'),
                val = parseInt(input.val()),
