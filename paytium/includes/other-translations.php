<?php
if ( ! defined( 'ABSPATH' ) ) {
	exit;
} // Exit if accessed directly

/**
 * Other translation strings for Paytium.
 *
 * @author        David de Boer
 * @since         1.4.0
 */

/**
 * Paytium PP main file
 */

__( 'Paytium Plus, Pro or Premium need the main plugin to work! Go to Plugins > Add new and search for "Paytium" to install it.', 'paytium' ) ;
__( 'You already have Paytium Plus, Pro or Premium enabled. You only need one of them, besides the Paytium main plugin.', 'paytium' );


/**
 * Paytium License Library
 */

__( '%s license:', 'paytium' );
__( 'Your license key', 'paytium' );
__( 'Your license key has expired, please renew it through our system', 'paytium' );
__( 'Your license has expired.', 'paytium' );
__( 'Please renew it to receive plugin updates.', 'paytium' );
__( 'Enter your license info and press Enter to activate it', 'paytium' );
__( 'Your license has been revoked', 'paytium' );
__( 'Invalid license key, please verify your license and try again', 'paytium' );
__( 'Your license is not active for this URL', 'paytium' );
__( 'This license is not valid for %s', 'paytium' );
__( 'Your license has reached its activation limit', 'paytium' );
__( 'Something went wrong, please verify everything and try again', 'paytium' );
__( 'There is a new version of %s available.', 'paytium' );
__( 'Please enter a valid license to receive this update.', 'paytium' );
__( 'Your license for %s is active.', 'paytium' );
__( 'Deactivate license', 'paytium');

/**
 * Paytium Other translations
 */
__( 'Status', 'paytium' );
__( 'Subscription', 'paytium' );
__( 'Amount', 'paytium' );
__( 'Total revenue', 'paytium' );
__( 'Start date', 'paytium' );
__( 'Renewal date', 'paytium' );
__( 'Subscription #%d', 'paytium' );
__( 'Subscription details', 'paytium' );
__( 'Subscription first payment', 'paytium' );
__( 'Subscription items', 'paytium' );
__( 'Customer details', 'paytium' );
__( 'Payments', 'paytium' );
__( 'A subscription processed by Paytium.', 'paytium' );
__( 'This subscription was created in test mode!', 'paytium' );
__( 'Subscription', 'paytium' );
__( 'ID', 'paytium' );
__( 'Transaction ID', 'paytium' );
__( 'Invoice ID', 'paytium' );
__( 'Payment date', 'paytium' );
__( 'Payment status', 'paytium' );
__( 'Order status', 'paytium' );
__( 'Amount', 'paytium' );
__( 'Description', 'paytium' );
__( 'Subscription', 'paytium' );
__( 'Subscription ID', 'paytium' );
__( 'Subscription interval', 'paytium' );
__( 'Subscription times', 'paytium' );
__( 'Subscription start date', 'paytium' );
__( 'Subscription payment status', 'paytium' );
__( 'Subscription first payment', 'paytium' );
__( 'Subscription recurring payment', 'paytium' );
__( 'Customer ID', 'paytium' );
__( 'Mode', 'paytium' );
__( 'Custom fields', 'paytium' );
__( 'Items', 'paytium' );
__( 'Read more about exporting payments.', 'paytium' );
__('Since Paytium 3.0 payments also have a payment source. This might influence the export of payments from before May 2019. %sRead more...%s', 'paytium' );
__( 'Paytium Export', 'paytium' );
__( 'Export payments', 'paytium' );
__( 'Read the Paytium MailChimp manual.', 'paytium' );
__( 'Don\'t show a "Subscribe to newsletter?" checkbox', 'paytium' );
__( 'If you check this, users will not see a "Subscribe to newsletter?" checkbox in the payment form, and will always be added to a newsletter (if all other conditions are met). If you leave this unchecked, users will see a "Subscribe to newsletter?" checkbox in the payment form and can check that checkbox if they want to be added to the newsletter. Showing an opt-in checkbox is mandatory by law in some cases.', 'paytium' );
__( 'Only subscribe after successful payment', 'paytium' );
__( 'If you only want people to be subscribed after a successful payment, check this. If you do not check this, people will be subscribed before payment.', 'paytium' );
__( 'API key', 'paytium' );
__( 'Go to MailChimp.com to get your API key', 'paytium' );
__( 'Your MailChimp Account', 'paytium' );
__( 'Below you will see the names and ID\'s of lists and groups you have in your MailChimp account. Use the ID\'s to let Paytium know how a subscriber should be added to your MailChimp account.', 'paytium' );
__( 'Default List ID', 'paytium' );
__( 'To what MailChimp List should subscribers be added by default? It\'s not required to set this here, it\'s okay if you only set the List ID in the shortcode. And you can always override the list ID in the shortcode for specific payment forms.', 'paytium' );
__( 'Default Group ID ', 'paytium' );
__( 'If you use Groups in MailChimp, set to what Group subscribers should be added by default. It\'s not required to set this here, it\'s okay if you only set the Group ID in the shortcode. And you can always override the group ID in the shortcode for specific payment forms.', 'paytium' );
__( 'Add your MailChimp API key above and Paytium will get your MailChimp account details. Saves you a lot of clicking :)', 'paytium' );
__( 'List:', 'paytium' );
__( '<b>Group category:</b>', 'paytium' );
__( 'Group:', 'paytium' );
__( 'Read the Paytium Zapier manual.', 'paytium' );
__( 'Enable integration', 'paytium' );
__( 'Enable or disable the integration with Zapier.', 'paytium' );
__( 'URL', 'paytium' );
__( 'Go to Zapier.com to get your webhook URL.', 'paytium' );
__( 'Statuses for webhook call', 'paytium' );
__( 'Choose payment statuses when webhook should be called', 'paytium' );
__( 'Read the Paytium ActiveCampaign manual.', 'paytium' );
__( 'Don\'t show a "Subscribe to newsletter?" checkbox', 'paytium' );
__( 'If you check this, users will not see a "Subscribe to newsletter?" checkbox in the payment form, and will always be added to a newsletter (if all other conditions are met). If you leave this unchecked, users will see a "Subscribe to newsletter?" checkbox in the payment form and can check that checkbox if they want to be added to the newsletter. Showing an opt-in checkbox is mandatory by law in some cases.', 'paytium' );
__( 'Only subscribe after successful payment', 'paytium' );
__( 'If you only want people to be subscribed after a successful payment, check this. If you do not check this, people will be subscribed before payment.', 'paytium' );
__( 'Add current status as tag', 'paytium' );
__( 'If you check this, the current payment status will be added to contacts as a tag.', 'paytium' );
__( 'Go to ActiveCampaign.com', 'paytium' );
__( 'and login, click on your account (top right), then "My Settings" and "Developer" to get your URL and API key.', 'paytium' );
__( 'URL', 'paytium' );
__( 'API key', 'paytium' );
__( 'Your ActiveCampaign Account', 'paytium' );
__( 'Below you will see the names and ID\'s of lists and also all tags you have in your ActiveCampaign account. Use the List ID\'s or Tag names to let Paytium know how a subscriber should be added to your ActiveCampaign account.', 'paytium' );
__( 'Default List ID', 'paytium' );
__( 'To what ActiveCampaign List should subscribers be added by default? It\'s not required to set this here, it\'s okay if you only set the List ID in the shortcode. And you can always override the list ID in the shortcode for specific payment forms.', 'paytium' );
__( 'Default Tags', 'paytium' );
__( 'If you use tags in ActiveCampaign: in this field enter what tags should be added to new subscribers by default. For example, add a tag \'payment\'. It\'s not required to set this here, it\'s okay if you only set the Tags in the shortcode. And you can always override the Tags in the shortcode for specific payment forms.', 'paytium' );
__( 'Add your ActiveCampaign API key above and Paytium will get your ActiveCampaign account details. Saves you a lot of clicking :)', 'paytium' );
__( 'Emails', 'paytium' );
__( 'Email', 'paytium' );
__( 'Emails', 'paytium' );
__( 'Email', 'paytium' );
__( 'Add New', 'paytium' );
__( 'Add New Email', 'paytium' );
__( 'New Email', 'paytium' );
__( 'Edit Email', 'paytium' );
__( 'View Email', 'paytium' );
__( 'Emails', 'paytium' );
__( 'Search Emails', 'paytium' );
__( 'Parent Emails:', 'paytium' );
__( 'No emails found.', 'paytium' );
__( 'No emails found in Trash.', 'paytium' );
__( 'Paytium Emails', 'paytium' );
__( 'Emails', 'paytium' );
__( 'Email updated.', 'paytium' );
__( 'Email updated.', 'paytium' );
__( 'Email restored to revision from %s', 'paytium' );
__( 'Email saved.', 'paytium' );
__( 'Email saved.', 'paytium' );
__( 'Subject', 'paytium' );
__( 'Email trigger(s)', 'paytium' );
__( 'Recipients', 'paytium' );
__( 'Status', 'paytium' );
__( 'ID', 'paytium' );
__( 'Only if an email field is found in the form.', 'paytium' );
__( 'Customer(s)', 'paytium' );
__( 'Email triggers', 'paytium' );
__( 'Email attachments', 'paytium' );
__( 'Tags for default information', 'paytium' );
__( 'Tags for custom fields', 'paytium' );
__( 'Looks like you don\'t have Paytium installed and activated, please fix that!', 'paytium' );
__( 'Place a first (test) payment and Paytium will show you what tags can be used as placeholders in emails.', 'paytium' );
__( 'These are all the default tags that you can use, based on the most recent payment #', 'paytium' );
__( 'Click on a tag to insert it into your content.', 'paytium' );
__( 'Default tags', 'paytium' );
__( 'Content', 'paytium' );
__( 'Subscription tags', 'paytium' );
__( 'Item tags', 'paytium' );
__( 'Looks like you don\'t have Paytium installed and activated, please fix that!', 'paytium' );
__( 'Place a first (test) payment and Paytium will show you what tags can be used as placeholders in emails.', 'paytium' );
__( 'These are the custom tags that you can use, based on most recent payment #', 'paytium' );
__( 'Click on a tag to insert it into your content.', 'paytium' );
__( 'Custom field:', 'paytium' );
__( 'Enable this email notification', 'paytium' );
__( 'Send to customer(s)', 'paytium' );
__( 'If an email address field is found within the payment form and the \'Send to customer(s)\' checkbox is checked, this email will be automatically sent to the customer\'s email address.', 'paytium' );
__( 'Other recipients:', 'paytium' );
__( 'List the (other) desired recipients for this email, one email address per line.', 'paytium' );
__( 'Enter email notification subject', 'paytium' );
__( 'Invoices', 'paytium' );
__( 'Tags for default information', 'paytium' );
__( 'Tags for custom fields', 'paytium' );
__( 'Start number', 'paytium' );
__( 'Invoice number prefix', 'paytium' );
__( 'Invoice number postfix', 'paytium' );
__( 'Invoice template', 'paytium' );
__( 'Download invoice #%s', 'paytium' );
__( 'invoice-%s.html', 'paytium' );
__( 'invoices', 'paytium' );
__( 'invoice-%s.html', 'paytium' );
__( 'Read more about exporting invoices.', 'paytium' );
__( 'Paytium Export Invoices', 'paytium' );
__( 'Export invoices', 'paytium' );
__( 'Invoice', 'paytium' );
__( 'Export invoices', 'paytium' );
__( 'View invoice #%s', 'paytium' );
__( 'Statistics', 'paytium' );
__( 'Revenue per day', 'paytium' );
__( 'Previous period', 'paytium' );
__( '6 months ago', 'paytium' );
__( '12 months ago', 'paytium' );
__( 'Payments last period', 'paytium' );
__( 'Payments', 'paytium' );
__( 'New subscriptions', 'paytium' );
__( 'Renewals next period', 'paytium' );
__( 'Payments today', 'paytium' );
__( 'Average per payment', 'paytium' );
__( 'Renewals last period', 'paytium' );
__( 'Payment methods', 'paytium' );
__( 'Recent Activity', 'paytium' );
__( 'ID', 'paytium' );
__( 'Status', 'paytium' );
__( 'Interval', 'paytium' );
__( 'Times', 'paytium' );
__( 'First payment', 'paytium' );
__( 'Renewal amount', 'paytium' );
__( 'Mollie ID', 'paytium' );
__( 'Customer', 'paytium' );
__( 'First payment', 'paytium' );
__( 'Start renewals', 'paytium' );
__( 'Cancelled', 'paytium' );
__( 'Note: subscription renewal payments that are already planned at Mollie can not be cancelled. Most of the time these are SEPA Direct Debit payments. You can choose to refund them manually after their status is confirmed. This can take up to 7 workdays.', 'paytium' );
__( 'Read the Paytium MailPoet manual.', 'paytium' );
__( 'Don\'t show a "Subscribe to newsletter?" checkbox', 'paytium' );
__( 'If you check this, users will not see a "Subscribe to newsletter?" checkbox in the payment form, and will always be added to a newsletter (if all other conditions are met). If you leave this unchecked, users will see a "Subscribe to newsletter?" checkbox in the payment form and can check that checkbox if they want to be added to the newsletter. Showing an opt-in checkbox is mandatory by law in some cases.', 'paytium' );
__( 'Only subscribe after successful payment', 'paytium' );
__( 'If you only want people to be subscribed after a successful payment, check this. If you do not check this, people will be subscribed before payment.', 'paytium' );
__( 'MailPoet Lists', 'paytium' );
__( 'Below you will see the names and ID\'s of lists in MailPoet. Use the ID\'s to let Paytium know how a subscriber should be added to MailPoet.', 'paytium' );
__( 'Default List ID', 'paytium' );
__( 'To what MailPoet List should subscribers be added by default? It\'s not required to set this here, it\'s okay if you only set the List ID in the shortcode. And you can always override the list ID in the shortcode for specific payment forms.', 'paytium' );
__( 'Redirects', 'paytium' );
__( 'Set a specific page redirect depending on the payment status. This is a global setting that can be overridden per form.', 'paytium' );
__( 'Redirect after %s', 'paytium' );